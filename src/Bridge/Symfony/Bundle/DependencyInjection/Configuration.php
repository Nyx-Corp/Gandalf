        ...
        $treeBuilder->getRootNode()
            ->children()
                ...
                ->arrayNode('email')
                    ...
                    ->children()
                        ->scalarNode('layout')
                            ->defaultValue('@_gandalf/email/layout.html.twig')
                            ->info('Twig template used as parent layout for security emails')
                        ->end()
                    ->end()
                ->end()
                ->arrayNode('credentials')
                    ...
                    ->children()
                        ->scalarNode('encryption_key')
                            ->defaultValue('%env(GANDALF_CREDENTIALS_KEY)%')
                            ->info('Base64-encoded 32-byte key for sodium_crypto_secretbox. Generate: php -r "echo base64_encode(sodium_crypto_secretbox_keygen());"')
                        ->end()
                    ->end()
                ->end()
                ->arrayNode('admin')
